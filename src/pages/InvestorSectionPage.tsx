...
export function InvestorSectionPage({ page }: { page: SectionPage }) {
  ...
  return <div className="section-page" dir="rtl"><header className="section-topbar"><a href="/"><img src="/assets/aaid-logo.png" alt="عائد" /></a><div><Search size={16} /><span>ابحث في مساحة المستثمر</span></div><a href="/start">مساهمة جديدة <ArrowLeft size={15} /></a></header><div className="section-layout"><InvestorWorkspaceNav active={page} /><main className="section-main"><SectionReveal className="section-intro"><div><span>{copy.eyebrow}</span><h1>{copy.title}</h1><p>{copy.description}</p></div><aside><ShieldCheck size={17} /><span>بيانات التجربة محفوظة محليًا</span></aside></SectionReveal>{page === "analysis" && <AnalysisExperience localTotal={localTotal} />}{page === "impact" && <ImpactExperience localTotal={localTotal} />}{page === "documents" && <DocumentsExperience />}{page === "preferences" && <PreferencesExperience />}</main></div></div>;
}
