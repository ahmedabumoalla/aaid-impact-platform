...
const paths = contributionOpportunities.map((opportunity) => ({
  ...opportunity,
  badge: opportunity.category,
  returnValue: opportunity.expectedReturn,
  impact: `${opportunity.beneficiaries} مستفيد`,
  risk: opportunity.sustainability,
}));

const steps = ["المبلغ", "الفرصة", "التأكيد"] as const;

function StartLogo() {
  return <a className="start-brand" href="/" aria-label="العودة إلى بوابة عائد"><img src="/assets/aaid-logo.webp" alt="عائد" decoding="async" /></a>;
}
...
export function StartContribution() {
  ...
  const [pathId, setPathId] = useState<string>(() => paths.some((item) => item.id === initialDraft?.pathId) ? initialDraft!.pathId : opportunityId);
  ...
  const selectedPath = paths.find((item) => item.id === pathId) ?? paths[0];
  ...
  const pathBeneficiaries = selectedPath.beneficiaries;
  ...
  const goNext = () => {
    ...
    if (step === 2) {
      ...
      recordDemoContribution({
        id,
        amount,
        opportunityId: selectedPath.id,
        opportunityTitle: selectedPath.title,
  ...
  const downloadReceipt = () => {
    ...
    const receipt = [
      ...
      `الفرصة المختارة ${selectedPath.title}`,
                ...
                {step === 1 && (
                  <>
                    <div className="path-step-heading">
                      <h1>نقترح لك <span>الفرصة الأنسب</span></h1>
                      <p>اعتمادًا على أهدافك وقيمة مساهمتك وتحليل الفرص بالذكاء الاصطناعي</p>
                    </div>
                    <div className="path-selection">
                      <motion.article className="path-recommendation selected" key={selectedPath.id} aria-label={`الفرصة المختارة ${selectedPath.title}`} initial={{ opacity: 0, y: reduceMotion ? 0 : 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0 : .24 }}>
                        <div className="path-recommendation-image"><img src={selectedPath.image} alt={selectedPath.imageAlt} decoding="async" /><span><Home size={12} aria-hidden="true" /> {selectedPath.category}</span></div>
                        <div className="path-recommendation-copy">
                          <span className="recommended-badge"><Sparkles size={12} aria-hidden="true" /> {selectedPath.id === opportunity.id ? "الفرصة الموصى بها" : "الفرصة المختارة"}</span>
                      ...
                      <div className="alternate-paths">
                        {alternatePaths.map((item) => (
                          <button aria-label={`اختيار ${item.title}`} className={pathId === item.id ? "selected" : ""} type="button" onClick={() => setPathId(item.id)} key={item.id}>
                            <div className="alternate-path-image"><img src={item.image} alt={item.imageAlt} loading="lazy" decoding="async" /><span>{item.badge}</span></div>
                ...
                {step === 2 && (
                  <>
                    <div className="review-step-heading">
                      ...
                      <p>تأكد من الفرصة المختارة وقيمة المساهمة ثم اختر وسيلة الدفع المناسبة</p>
                    </div>

                    <article className="review-selected-path">
                      <div className="review-selected-image"><img src={selectedPath.image} alt={selectedPath.imageAlt} decoding="async" /><span><Home size={12} aria-hidden="true" /> {selectedPath.category}</span></div>
                      <div className="review-selected-copy">
                        <span className="review-selected-badge"><CheckCircle2 size={13} aria-hidden="true" /> الفرصة المختارة</span>
                        <h2>{selectedPath.title}</h2>
                        <p>{selectedPath.description}</p>
                        <PathImpactMetrics path={selectedPath} />
                        <button type="button" onClick={() => setStep(1)}><Pencil size={13} aria-hidden="true" /> تعديل الفرصة</button>
                      </div>
                    </article>
                    ...
                    <section className="review-contribution" aria-labelledby="review-contribution-title">
                      ...
                      <dl>
                        ...
                        <div><dt>المدة المستهدفة</dt><dd>{selectedPath.duration}</dd></div>
                      </dl>
                    </section>
                ...
                {step === 3 && (
                  <div className="success-state">
                    <div className="success-progress" aria-label="اكتملت رحلة المساهمة">
                      {["قيمة المساهمة", "الفرصة المختارة", "المراجعة", "النتيجة"].map((label, index) => (
                    ...
                    <section className="success-hero" aria-labelledby="success-title">
                      ...
                      <div><span>تم اعتماد مساهمتك بنجاح</span><h1 id="success-title">بدأ أثرك الآن</h1><p>شكرا لك فأنت تصنع أثرا يستمر وينمو</p><a className="success-dashboard" href={completion ? `/investor/dashboard?contribution=${encodeURIComponent(completion.id)}` : "/investor/dashboard"}>الانتقال إلى لوحة المستثمر <ArrowLeft size={17} aria-hidden="true" /></a></div>
                    </section>
                    ...
                    <section className="success-receipt-card" aria-label="تفاصيل المساهمة">
                      ...
                      <div className="receipt-detail"><span><Home size={19} /></span><div><small>الفرصة المختارة</small><strong>{selectedPath.title}</strong></div></div>
                      <div className="receipt-detail"><span><Clock3 size={19} /></span><div><small>المدة المتوقعة</small><strong>{selectedPath.duration}</strong></div></div>
                    ...
                    <section className="future-projection" aria-labelledby="future-projection-title">
                      ...
                      <figure><img src={selectedPath.image} alt={selectedPath.imageAlt} loading="lazy" decoding="async" /><figcaption><Home size={16} /> {selectedPath.title}</figcaption></figure>
                    </section>
          ...
          {step < 3 && (
            <>
              ...
              <div className="start-navigation">
                <button className="start-primary-action" type="button" onClick={goNext} disabled={!canContinue}>{step === 2 ? "تأكيد المساهمة" : step === 1 ? "اعتماد هذه الفرصة" : "التالي"}<ArrowLeft size={17} /></button>
...
function PathDetailsModal({ path, amount, onClose, reduceMotion }: { path: (typeof paths)[number]; amount: number; onClose: () => void; reduceMotion: boolean }) {
  ...
  const allocation = path.allocation;
    ...
    <motion.div className="path-modal-backdrop" role="presentation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduceMotion ? 0 : .2 }} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <motion.div ref={dialogRef} className="path-modal" role="dialog" aria-modal="true" aria-labelledby="path-modal-title" initial={{ opacity: 0, y: reduceMotion ? 0 : 24, scale: reduceMotion ? 1 : .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: reduceMotion ? 0 : 16, scale: reduceMotion ? 1 : .99 }} transition={{ duration: reduceMotion ? 0 : .25 }}>
        ...
        <div className="path-modal-hero">
          <div className="path-modal-image"><img src={path.image} alt={path.imageAlt} decoding="async" /><span>{path.category}</span></div>
          <div className="path-modal-title">
            ...
            <div><span><MapPin size={14} aria-hidden="true" /> {path.district}</span><span><CalendarDays size={14} aria-hidden="true" /> مدة مستهدفة {path.duration}</span></div>
          </div>
        </div>
        ...
        <div className="path-modal-footer"><div><ShieldCheck size={17} aria-hidden="true" /><span><strong>إفصاح النموذج الأولي</strong><small>المؤشرات تقديرية لعرض تجربة المنتج ولا تمثل ضمانا للعائد</small></span></div><button type="button" onClick={onClose}>العودة إلى الفرصة</button></div>
      </motion.div>
    </motion.div>
  );
}
